...
- **侧栏会话提示：** 命令面板关闭时，按住 Alt/Option 会在侧栏前九个完整可见会话上显示同一套快捷键；继续按 Shift 和数字即可打开会话，松开 Alt/Option 后提示消失。首次使用时会显示可关闭的引导提示；按住 Alt/Option 成功显示快捷键后，引导会自动完成并永久隐藏。
...
2. 输入关键词筛选命令和操作。
3. 使用上下方向键、Control-N/P 或 Control-J/K 与 Enter；按 Alt/Option-Shift-1–9 直接执行带编号的结果；或直接点击结果。
4. 命令面板关闭时，按首次引导或直接按住 Alt/Option 显示侧栏会话快捷键，再加按 Shift 和 1–9 打开对应会话。
5. 点击底部的「快捷键」并按下新组合键，即可修改快捷键。
6. 点击「恢复默认」可还原平台默认值。
